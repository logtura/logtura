...
/** Exposed so callers (e.g. the kitchen-sink build script) can render
 *  a Dockerfile from an arbitrary union of driver dockerfileDeps
 *  without going through generateBundle.
 *
 *  Pass `mountVectorYamlAtRuntime: true` to omit the `COPY vector.yaml`
 *  line — used by the kitchen-sink build where vector.yaml is mounted
 *  by the deploy target (Fly's `config.files`) at machine start, not
...
export function renderDockerfile(
  ...
  const installSteps = deps
    .map((d) => {
      if (d.directive) return d.directive;
      if (d.install) return `RUN ${d.install}`;
      return null;
    })
